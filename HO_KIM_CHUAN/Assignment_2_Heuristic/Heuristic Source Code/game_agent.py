...
def custom_score(game, player):
    '''
    This heuristic method is based on a offensive to defensive method. I reused the
    weighted_chances_heuristic to maintain the good result while split the usage of two
    different heuristic function in different phase of a game so that the player tend to play
    more offensive to kill the opponent, and play more defensive when the game is almost finsihing
    to reduce the probability of losing.
    '''
    ...
    leftover_spaces = len(game.get_blank_spaces())
    total_spaces = float(game.width * game.height)

    current_progress = leftover_spaces/total_spaces
...
def aggressive_heuristic(game, player):
    ...
    # print("my_move: ", my_moves)
    opponent_moves = len(game.get_legal_moves(game.get_opponent(player)))
    # print('opponent: ',opponent_moves)
    return my_moves - 1.5 * opponent_moves
